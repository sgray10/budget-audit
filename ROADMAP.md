...
## Current checkpoint — 2026-06-30 packet, pages 23-85

The current working slice covers the Weakley County Finance, Ways, and Means packet budget pages 23-85. The pipeline can render/OCR pages, extract OCR rows, enrich with page-review metadata, classify rows, apply manual corrections, create review queues, summarize, and reconcile selected funds.

Known validated reconciliation checkpoint:

| Fund | Status | Notes |
|---:|---|---|
| 101 | Reconciled close to source totals | Net after transfers: -4,407. |
| 116 | Reconciled to extracted source totals | Small planned use of fund balance. |
| 122 | Reconciled to extracted source totals | Small planned use of fund balance. |
| 131 | Reconciled after correction overlay | Revenue 7,909,610; expenditures 7,868,699; net 40,911. |

Next best milestones:

1. Update the workflow documentation to reflect the corrected 23-85 pipeline.
2. Add a single runner command or script for the reviewed packet range.
3. Extend extraction/page-review metadata into Fund 141.
4. Add subtotal-level reconciliation so mismatches are found automatically instead of by ad hoc scripts.
5. Begin analysis outputs: deltas, material changes, compensation summaries, and public questions.
