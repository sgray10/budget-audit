...
As of 2026-07-04, this spec's v3 (the report-refinement pass, issue #35, on top of v2's civic-intelligence review packet restructure, issue #32) is implemented against all 10 reconciled funds (101/116/122/131/141/143/151/171/172/202). The v1 status (8-category taxonomy, prefix clustering, data-quality warnings, top changes) described in earlier revisions of this section is superseded by the v2/v3 notes below.
...
- **Clustering**: unchanged core mechanism (fund + label-prefix grouping, `clusters.py`), tagged with a `cluster_type` (`debt_service`, `grant_funded_capital_project`, `mixed_grant_program`, `grant_funded_program`, `allocation_program`, `personnel_or_benefits`, `general`) derived from the majority `classify.categorize_line_item` category among a cluster's member rows, plus a one-paragraph plain-English `narrative` field. Recipient-allocation as the majority category is checked before grant-revenue pairing, so a cluster like `OPID` (opioid settlement revenue funding named city/nonprofit recipients) resolves to `allocation_program`, not `grant_funded_program`, even though its revenue side is grant-classified. **v3**: `grant_funded_capital_project` now requires a material capital-dollar anchor (>=80% of cluster expenditure is capital-classified), not just a paired grant-revenue line -- `141-ISM` (73% capital, real personnel/benefits spending alongside it) and `141-PDG` (59% capital) were being over-classified as capital projects under v2's category-presence check and now correctly resolve to the new `mixed_grant_program` type (<=20% capital falls through to plain `grant_funded_program`). Each cluster also now carries `key_revenue_line`/`key_expenditure_line` (page/account/label/amount references) so evidence rendering doesn't have to say only `cluster_id=...`, and a one-sided cluster's narrative appends an explicit note that the missing offsetting side may be in another fund/transfers/debt proceeds/a non-extracted schedule. Validated against real clusters: `151-JAIL`/`151-USDA`/`151-HVAC`/`151-CRT` (debt), `101-OPID` (allocation), `101-CRD`/`101-ISIP` (grant-funded capital, ~100% capital share), `141-ISM`/`141-PDG` (mixed), `141-ABE`/`141-SUM`/`141-TRN` (grant-funded program, ~0% capital share). Semantic-similarity clustering is still not implemented.
- **Semantic cross-linking (v3)**: new (`related_items.py`) -- each priority area gets a list of "potentially related items" found elsewhere in the packet, via three generic (not hardcoded-pair) mechanisms: fund-name search (a whole-fund change to "Nursing Home" finds Fund 101's "NH Investment Income - Nursing Home Funds" and "NH Contributions - Nursing Home" because the fund's name appears in those labels), cluster-prefix-stem similarity (`OPID` <-> `OPIA`, both opioid-settlement prefixes), and packet-rare shared keywords (a capitalized 6+ letter word appearing in <=4 rows packet-wide, after stripping recipient-name phrases like "City of Dresden" so a recipient's city name can't leak in as a program keyword). Rendered with explicit "the packet alone does not confirm a connection" language -- never asserted as definite. The keyword stoplist was tuned against a real generated report: "health" and "patient" both cleared the rarity gate but produced coincidental, non-substantive links (Fund 171's "Other Health & Welfare Grants" to an unrelated school-health program via "Health" alone; Fund 202's "Patient Charges" to jail "Inmate/Patient Charges" via "Patient" alone) and were added to the stoplist as a result.
...
- **Data-quality impact scoring**: new (`data_quality.data_quality_impact_score`) -- combines severity, confidence, dollar amount, and whether the row shows up in a top-dollar change or a priority-typed cluster into one score; warnings at or above `HIGH_IMPACT_THRESHOLD` surface in the report's main body, everything else moves to Appendix B. This is what keeps the ~40-75 routine `$1`-placeholder and manual-correction-dependency warnings per report from flooding the main body while still surfacing the handful (single digits, in this packet) that actually touch a number the report is putting in front of a reader. **v3**: added `label_corruption_reason()`, a narrow heuristic for genuinely OCR-mangled labels (broken-word lowercase fragments amid Title Case text; stray quote artifacts) -- validated against the full real dataset before being kept: across 657 distinct labels it flags exactly the two known-corrupted ones ("CRD Other State lal eral Development" and "tisa 'N Investment yore Achievement") and nothing else. A corrupted label carrying a large dollar amount gets an extra score bump (+10) in `data_quality_impact_score`, and `priority_areas.py` now also renders an inline "this label appears OCR-corrupted" caveat directly inside any priority area that quotes a corrupted label (a reader hits the priority-areas section before the data-quality section, so quoting a garbled label there without a caveat would read as a verified name).
...
- **Priority follow-up areas**: new (`priority_areas.py`) -- synthesizes a ranked, capped list (default 10) from whole-fund changes, fund-level grant/capital pairs, notable-typed clusters, and high-impact data-quality warnings, not from every individual finding. Reserves minimum slots for debt-service and allocation-program clusters so a handful of large grant/capital pairs can't crowd out smaller but still-important patterns on pure dollar magnitude alone, and dedupes a cluster-level `grant_funded_capital_project` against a fund-level pair for the same fund. **v3**: each area now also carries a `why_normal` sentence (a neutral, plausible ordinary explanation per pattern type -- e.g. "may reflect activity moving to another fund, facility closure, privatization, reclassification, or a change in accounting presentation"), a `first_records_request` (one concrete, ready-to-send document request per pattern, distinct from the open-ended `questions` list), the semantic cross-linking described above, and richer human-readable evidence (per-line page/account/label/old-value/new-value references, not just a bare `cluster_id=...`).
...
- **Not yet implemented**: fund-size-relative materiality ranking (still absolute-dollar + percent only), config-driven multi-county jurisdiction profiles, true semantic-similarity clustering (v3's cross-linking is fund-name/prefix-stem/rare-keyword matching, not embeddings), and automated staleness guardrails for page-review/correction files (see `docs/milestones.md` v0.4).
- **v3 calibration caveat**: the capital-share thresholds in `clusters.py` (>=80% capital -> capital project, <=20% -> plain program) and the keyword-rarity threshold in `related_items.py` (<=4 packet-wide occurrences) were calibrated against this one packet's real clusters and label vocabulary, not derived analytically. A future packet/county with a different label vocabulary or cluster-size distribution may need these thresholds (and the keyword stoplist) re-checked against its own real data before being trusted, per this project's "verify against real data" discipline.
